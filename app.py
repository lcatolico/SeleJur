...
def conectar_sheets():
    ...
    if "sheet_id" in st.secrets and st.secrets["sheet_id"]:
        pl = cl.open_by_key(st.secrets["sheet_id"])
    elif "sheet_name" in st.secrets and st.secrets["sheet_name"]:
...
CH_COL_VIDEO_CANDIDATOS = 25
CH_COL_VIDEO_PERGUNTAS = 26
CH_COL_VIDEO_PRAZO = 27
CH_COL_VIDEO_STATUS = 28
...
garantir_coluna(aba_chamadas, "video_candidatos", CH_COL_VIDEO_CANDIDATOS)
garantir_coluna(aba_chamadas, "video_perguntas", CH_COL_VIDEO_PERGUNTAS)
garantir_coluna(aba_chamadas, "video_prazo", CH_COL_VIDEO_PRAZO)
garantir_coluna(aba_chamadas, "video_status", CH_COL_VIDEO_STATUS)
...
LETRAS_DISC = ["D","I","S","C"]
DISCLAIMER = "O SeleJur atua exclusivamente como plataforma de apoio à organização de Seletivos jurídicos. A publicação deste Seletivo não configura processo seletivo vinculante, concurso público ou compromisso de contratação. O uso do ius indicandum é de responsabilidade exclusiva do recrutador, que deve observar as normas de impessoalidade e vedação ao nepotismo aplicáveis ao seu órgão."
...
def auth_salt():
    try:
        return st.secrets.get("auth", {}).get("salt", "selejur-session")
    except Exception:
        return "selejur-session"
...
def auth_cand_query():
    ...
    if not token:
        token = st.query_params.get("cauth", "")
        if isinstance(token, list):
            token = token[0] if token else ""
...
def ir(p):
    ...
    if p == "chamadas" and st.session_state.get("seletivo_pendente_sid"):
        st.query_params["sid"] = st.session_state.seletivo_pendente_sid
    st.rerun()
...
def link_publico_seletivo(ch):
    sid = str(ch.get("id","")).strip()
    if not sid:
        return ""
    return f"https://jurisbank.streamlit.app/?p=chamadas&sid={urllib.parse.quote(sid)}"

def triagem_assistida_candidato(c, ch):
    requisitos = str(ch.get("requisitos","")).lower()
    texto_cand = " ".join([
        str(c.get("formacao","")),
        str(c.get("area","")),
        str(c.get("experiencia","")),
        str(c.get("experiencias","")),
        str(c.get("sistemas","")),
        str(c.get("resumo","")),
    ]).lower()
    sinais = []
    if c.get("oab") == "Sim":
        sinais.append("OAB informada")
    if c.get("disponibilidade") == "Sim":
        sinais.append("disponibilidade declarada")
    if c.get("disc"):
        sinais.append(f"perfil comportamental {c.get('disc')}")
    for termo in ["gabinete","assessoria","eproc","saj","pje","minuta","audiencia","tribunal","ministerio publico","defensoria"]:
        if termo in requisitos and termo in texto_cand:
            sinais.append(f"aderencia a {termo}")
    pontos = sinais[:4] or ["informacoes suficientes para triagem inicial"]
    alertas = []
    if c.get("oab") != "Sim":
        alertas.append("confirmar situacao de OAB se for requisito")
    if not c.get("resumo"):
        alertas.append("curriculo/resumo ainda pouco estruturado")
    if not c.get("disc"):
        alertas.append("perfil comportamental nao preenchido")
    proxima = "Entrevista estruturada"
    if "prova" in requisitos or "minuta" in requisitos or "escrita" in requisitos:
        proxima = "Prova pratica ou teste de escrita"
    elif "experiencia" in requisitos:
        proxima = "Entrevista com estudo de caso"
    perguntas = [
        "Quais experiencias anteriores mais se aproximam das demandas deste Seletivo?",
        "Como organizaria as prioridades nas primeiras semanas de atuacao?",
        "Que ponto do curriculo exige confirmacao tecnica na entrevista?",
    ]
    return {
        "pontos": pontos,
        "alertas": alertas[:3] or ["sem alerta critico a partir dos dados informados"],
        "proxima": proxima,
        "perguntas": perguntas,
    }

def status_candidato_no_seletivo(c, ch):
    email = str(c.get("email","") or "").strip()
    video_convocados = [e.strip() for e in str(ch.get("video_candidatos","")).split(",") if e.strip()]
    selecionados = [e.strip() for e in str(ch.get("selecionados","")).split(",") if e.strip()]
    status_ch = str(ch.get("status","") or "").strip().lower()
    if email in selecionados:
        return "Selecionado"
    if email in video_convocados:
        return "Pre-entrevista em video"
    if status_ch in ["encerrado","deserto","interrompido"]:
        return "Nao selecionado/encerrado"
    return "Inscrito"
...
def salvar_video_seletivo(aba, linha, dados: dict):
    MAPA = {
        "video_candidatos": CH_COL_VIDEO_CANDIDATOS,
        "video_perguntas": CH_COL_VIDEO_PERGUNTAS,
        "video_prazo": CH_COL_VIDEO_PRAZO,
        "video_status": CH_COL_VIDEO_STATUS,
    }
    updates = []
    for campo, col in MAPA.items():
        if campo in dados:
...
def email_resultado_seletivo(destinatario, nome_candidato, titulo, orgao, situacao, mensagem):
    assunto = f"SeleJur — Resultado do Seletivo: {titulo}"
    corpo = f"""
    <div style="font-family:'Segoe UI',Arial,sans-serif;max-width:640px;margin:0 auto;background:#ffffff;color:#071D49">
        <div style="background:#071D49;padding:28px;border-radius:14px 14px 0 0">
            <h1 style="color:#ffffff;font-size:22px;margin:0">SeleJur</h1>
...
def email_redefinicao(destinatario, link, tipo):
    ...
    assunto = "SeleJur — Redefinição de senha"
    corpo = f"""
    <div style="font-family:'Segoe UI',Arial,sans-serif;max-width:600px;margin:0 auto;background:#ffffff">
        ...
        <div style="padding:32px;background:#FBFAF7;border-radius:0 0 12px 12px">
            ...
            <p style="color:#5E6675;font-size:15px;line-height:1.7;margin:0 0 20px">
                Recebemos uma solicitação de redefinição de senha para sua conta de <strong style="color:#071D49">{label}</strong> no SeleJur.
            </p>
            ...
            <p style="color:#9a9a9a;font-size:12px;line-height:1.6;margin:0;border-top:1px solid #D9D2C3;padding-top:16px">
                ...
                <strong>SeleJur</strong> — Seletivos jurídicos com triagem assistida.
            </p>
        </div>
    </div>
    """
...
def email_recomendador(nome_candidato, email_recomendador, link):
    assunto = f"SeleJur — {nome_candidato} solicitou sua avaliação"
    corpo = f"""
    <div style="font-family:'Segoe UI',Arial,sans-serif;max-width:600px;margin:0 auto;background:#ffffff">
        <div style="background:linear-gradient(135deg,#071D49,#071D49);padding:32px;text-align:center;border-radius:12px 12px 0 0">
            ...
            <h1 style="color:#ffffff;font-size:22px;margin:0;letter-spacing:-0.5px">SeleJur</h1>
        ...
        <div style="padding:32px;background:#f4f6fc;border-radius:0 0 12px 12px">
            ...
            <p style="color:#4a5568;font-size:15px;line-height:1.7;margin:0 0 20px">
                O profissional <strong style="color:#071D49">{nome_candidato}</strong> indicou você como recomendador no SeleJur e solicita que você preencha uma avaliação do seu perfil profissional.
            </p>
            ...
            <p style="color:#8090b8;font-size:12px;line-height:1.6;margin:0;border-top:1px solid #D9D2C3;padding-top:16px">
                ...
                <strong>SeleJur</strong> — plataforma de Seletivos jurídicos com apoio à triagem.
            </p>
        </div>
    </div>
    """
...
nav_html = '<div class="topbar"><a class="topbar-logo" href="https://lcatolico.github.io/jurisbank/"><div class="topbar-logo-icon">SJ</div><div><span class="topbar-logo-name">SeleJur</span></div></a><div class="topbar-nav">'
...
if pagina == "publico":
    st.markdown("""<div class="hero-card">
        <h1 class="page-title">SeleJur<br><em>Ambientes.</em></h1>
        ...
        with tabs[0]:
            ...
            if entrar_login:
                if not email_login:
                    ...
                    if st.session_state.get("seletivo_pendente_sid"):
                        st.query_params["p"]="chamadas"
                        st.query_params["sid"]=st.session_state.seletivo_pendente_sid
                    else:
                        st.query_params["p"]="inicio"
                    st.query_params["cauth"]=st.session_state.cand_auth_token
                    st.success("Bem-vindo ao SeleJur.")
                    st.rerun()
...
# ── PÁGINA: CHAMADAS ──────────────────────────────────────────────────────────
    ...
    if isinstance(sid_param, list): sid_param = sid_param[0]
    if sid_param:
        st.session_state.seletivo_pendente_sid = sid_param
    restaurar_candidato_da_sessao()
    email_param = params.get("email", "")
    if isinstance(email_param, list):
        email_param = email_param[0]
    ...
    if sid_param and not cand_logado() and not rec_logado():
        st.markdown('<div class="info-card">Ainda nao tem cadastro? Crie seu perfil para concluir a inscricao neste Seletivo.</div>', unsafe_allow_html=True)
        if st.button("Criar cadastro para este Seletivo", key="btn_cadastro_sid"):
            st.session_state.seletivo_pendente_sid = sid_param
            ir("cadastro")
    ...
    chf=todas
    if sid_param:
        chf=[ch for ch in chf if str(ch.get("id","")) == str(sid_param)]
    ...
    if sid_param:
        chf=[ch for ch in todas if str(ch.get("id","")) == str(sid_param)]

    if sid_param and not chf:
        st.warning("Este link de Seletivo nao foi encontrado ou nao esta mais disponivel.")
    ...
    for i,ch in enumerate(chf):
        ...
        email_logado = st.session_state.cand_logado.get("email","") if cand_logado() else ""
        video_convocados = [e.strip() for e in str(ch.get("video_candidatos","")).split(",") if e.strip()]
        if ja and email_logado in video_convocados:
            st.markdown(f"""<div class="info-card" style="margin-top:8px">
                <strong>Pre-entrevista em video solicitada</strong><br>
                Prazo: {html_texto(ch.get("video_prazo","a definir"))}<br><br>
                <strong>Perguntas:</strong><br>{html_texto(ch.get("video_perguntas",""))}
                <div class="disclaimer-box">Nesta versao de validacao, o SeleJur registra a solicitacao. O envio direto do video sera ativado na etapa com armazenamento proprio.</div>
            </div>""", unsafe_allow_html=True)
...
# ── PÁGINA: CADASTRO ──────────────────────────────────────────────────────────
    ...
    if et==1:
        ...
        with c2:
            if st.button("Cadastrar no SeleJur"):
                if None in rd: st.error(f"Responda todas as {len(PERGUNTAS_DISC)} perguntas.")
                    ...
                    try:
                        ...
                        cand_sessao = {
                            **d,
                            "selo_verificado": selos["verificado"],
                            "selo_recomendado": selos["recomendado"],
                            "selo_destaque": selos["destaque"],
                            "selo_experiente": selos["experiente"],
                            "disc": pd_,
                        }
                        st.session_state.cand_logado = cand_sessao
                        st.session_state.cand_email_logado = d["email"]
                        st.session_state.cand_auth_token = token_candidato(cand_sessao)
...
# -- PAGINA: CADASTRO REALIZADO ------------------------------------------------
    ...
    with c1:
        if st.button("Entrar na Area do Candidato",key="cad_sucesso_login"):
            ir("inicio")
    with c2:
        botao_chamadas = "Continuar inscricao no Seletivo" if st.session_state.get("seletivo_pendente_sid") else "Ver Seletivos disponiveis"
        if st.button(botao_chamadas,key="cad_sucesso_chamadas"):
            ir("chamadas")
        ...
        if st.session_state.rec_dashboard in ["novo_seletivo","seletivos"]:
            ...
            if st.session_state.rec_dashboard == "novo_seletivo":
                ...
                for i,ch in enumerate(mch):
                    ...
                    link_publico = link_publico_seletivo(ch)
                    if link_publico:
                        st.text_input("Link publico do Seletivo", value=link_publico, key=f"link_publico_{ch_key}", disabled=True)
                        ...
                        with st.expander("Resultado e encerramento do Seletivo", expanded=True):
                            ...
                            with st.form(f"form_resultado_chamada_{i}"):
                                tipo_resultado = st.selectbox(
                                    ...
                                    [
                                        "Selecionado(s) entre candidatos do SeleJur",
                                ...
                                selecionados_opcoes = []
                                if tipo_resultado == "Selecionado(s) entre candidatos do SeleJur":
                                ...
                                msg_reprovados = st.text_area(
                                    ...
                                    value="Agradecemos sua participação. Neste momento, seu perfil não foi selecionado para este Seletivo. Você continuará podendo participar de novas oportunidades compatíveis no SeleJur.",
                                    height=110,
                            ...
                            if confirmar_resultado:
                                if tipo_resultado == "Selecionado(s) entre candidatos do SeleJur" and not selecionados_opcoes:
                    ...
                    if st.session_state.get(painel_inscritos_key):
                        with st.expander("Painel de inscritos",expanded=True):
                            if not ins_: st.info("Nenhum inscrito.")
                                ...
                                st.markdown('<p class="section-label">Pre-entrevista em video</p>',unsafe_allow_html=True)
                                st.markdown('<div class="info-box">Selecione candidatos para uma fase de video. O prototipo registra perguntas e prazo; o upload direto de videos deve usar armazenamento proprio na versao com Supabase.</div>',unsafe_allow_html=True)
                                opcoes_video = [f"{c.get('nome','Sem nome')} <{c.get('email','')}>" for c in id_]
                                mapa_video = {f"{c.get('nome','Sem nome')} <{c.get('email','')}>": c.get("email","") for c in id_}
                                video_atuais = [e.strip() for e in str(ch.get("video_candidatos","")).split(",") if e.strip()]
                                default_video = [op for op,email in mapa_video.items() if email in video_atuais]
                                with st.form(f"form_video_{ch_key}"):
                                    selecionados_video = st.multiselect("Candidatos para pre-entrevista em video", opcoes_video, default=default_video, key=f"video_sel_{ch_key}")
                                    perguntas_video = st.text_area(
                                        "Perguntas do video",
                                        value=ch.get("video_perguntas","") or "1. Conte brevemente sua experiencia mais aderente a este Seletivo.\n2. Como voce organizaria suas primeiras semanas no cargo?\n3. Qual desafio juridico ou institucional voce se sente mais preparado para enfrentar?",
                                        height=120,
                                        key=f"video_perg_{ch_key}",
                                    )
                                    prazo_video = st.text_input("Prazo para envio do video", value=ch.get("video_prazo",""), placeholder="dd/mm/aaaa", key=f"video_prazo_{ch_key}")
                                    salvar_video = st.form_submit_button("Salvar solicitacao de video")
                                if salvar_video:
                                    if not selecionados_video:
                                        st.error("Selecione ao menos um candidato para a fase de video.")
                                    elif not perguntas_video.strip():
                                        st.error("Informe as perguntas do video.")
                                    else:
                                        tc_ch = leitura_planilha_segura(
                                            registros_chamadas_cache,
                                            "sheet_error_chamadas",
                                            "NÃ£o foi possÃ­vel consultar os Seletivos agora. Tente novamente em alguns instantes.",
                                        )
                                        idx_ch=next((j for j,c in enumerate(tc_ch) if c.get("id")==ch.get("id")),None)
                                        if idx_ch is None:
                                            st.error("NÃ£o encontrei este Seletivo na planilha.")
                                        else:
                                            payload_video = {
                                                "video_candidatos": ", ".join([mapa_video[o] for o in selecionados_video if mapa_video.get(o)]),
                                                "video_perguntas": perguntas_video.strip(),
                                                "video_prazo": prazo_video.strip(),
                                                "video_status": "solicitado",
                                            }
                                            salvar_video_seletivo(aba_chamadas, idx_ch+2, payload_video)
                                            st.success("Solicitacao de pre-entrevista em video registrada.")
                                            st.rerun()

                                st.markdown('<p class="section-label">Perfis</p>',unsafe_allow_html=True)
                                st.markdown('<div class="info-box">A triagem abaixo e um apoio inicial para leitura dos inscritos. Ela nao aprova, reprova ou substitui entrevista, prova pratica ou decisao humana.</div>',unsafe_allow_html=True)
                                for cand in id_:
                                    cor=cor_avatar(cand["nome"])
                                    resumo_c = resumo_card_candidato(cand)
                                    triagem = triagem_assistida_candidato(cand, ch)
                                    status_cand = status_candidato_no_seletivo(cand, ch)
                                    pontos_html = "".join([f'<span class="profile-chip profile-chip-green">{html_texto(p)}</span>' for p in triagem["pontos"]])
                                    alertas_html = "".join([f'<span class="profile-chip profile-chip-gold">{html_texto(a)}</span>' for a in triagem["alertas"]])
                                    perguntas_html = "".join([f'<p class="cand-sub">- {html_texto(p)}</p>' for p in triagem["perguntas"]])
                                    st.markdown(f"""<div class="cand-card" style="margin-bottom:8px">
                                        <div style="display:flex;align-items:center;gap:12px">
                                            ...
                                            <div style="flex:1">
                                                <p class="cand-name">{cand['nome']}</p>
                                                <div style="margin-bottom:6px"><span class="profile-chip">{html_texto(status_cand)}</span></div>
                                                <p class="cand-sub">{cand.get('formacao','—')} · {cand.get('area','—')}</p>
                                                <p class="cand-sub"><strong>Experiencia:</strong> {html_texto(resumo_c.get('experiencia','-'))}</p>
                                                <p class="cand-sub"><strong>Sistemas:</strong> {html_texto(resumo_c.get('sistemas','-'))}</p>
                                                <div style="margin-top:4px">{html_selos(cand)}{html_disc(cand)}</div>
                                                <div style="margin-top:10px"><p class="cand-sub"><strong>Pontos de aderencia</strong></p>{pontos_html}</div>
                                                <div style="margin-top:8px"><p class="cand-sub"><strong>Alertas para conferir</strong></p>{alertas_html}</div>
                                                <div style="margin-top:8px"><p class="cand-sub"><strong>Proxima etapa sugerida:</strong> {html_texto(triagem["proxima"])}</p></div>
                                                <div style="margin-top:8px"><p class="cand-sub"><strong>Perguntas sugeridas</strong></p>{perguntas_html}</div>
                                            </div>
    ...
    if not token_url:
        ...
        if not rec_recom:
            ...
            st.markdown(f'''<div class="hero-card">
                ...
                <p class="page-sub">Você foi indicado como recomendador de <strong style="color:#C49A2C">{nome_cand}</strong> no SeleJur. Preencha a avaliação abaixo.</p>
            </div>''', unsafe_allow_html=True)
...
# ── PÁGINAS LEGAIS ────────────────────────────────────────────────────────────
    ...
    if ip:
        ...
        secs=[
            ("1. Aceitação",[("p","Ao usar o SeleJur você concorda com estes Termos e com a Política de Privacidade.")]),
            ...
            ("4. Responsabilidades",[("p","O SeleJur não garante contratações e não se responsabiliza por decisões dos recrutadores.")]),
    ...
    for ts,cont in secs:
        ...
        st.markdown('<div class="custom-divider"></div>',unsafe_allow_html=True)
    st.markdown(f'<p style="font-size:12px;color:#5E6675;font-weight:500;text-align:center">SeleJur — Versão 1.0 — {datetime.now().strftime("%d/%m/%Y")}</p>',unsafe_allow_html=True)
